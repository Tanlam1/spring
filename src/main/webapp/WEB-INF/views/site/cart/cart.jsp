...
<div class="row">
    <c:if test="${empty cartItems}">
        <div class="col-lg-8">
            <span style=" width: 100%; text-align: center; display: block; margin: 10px 0 20px; font-size: 16px; color: #e41378; font-weight: bold; ">Giỏ hàng rỗng, hãy mua hàng rồi quay lại sau nhé! <a href="/list">Xem sản phẩm</a></span>
        </div>
    </c:if>
                    ...
                    <div class="card-header bg-white border-0 p-0" id="headingOne">
                        <h2 class="mb-0">
                            <button
                                ...
                                <span class="c-number">1</span> Cart (${cartItems.size()} items)
                            </button>
                        </h2>
                    </div>
                            ...
                            <div class="osahan-cart">
                                <c:forEach var="item" items="${cartItems}">
                                    <div class="cart-items bg-white position-relative border-bottom">
                                        <a href="/cart/remove/${item.cartItemId}" class="remove__cart">Remove</a>
                                        <input onchange="window.location.href = '/cart/update/select/${item.cartItemId}'" class="select__cart" type="checkbox" name="selected" ${item.selected ? 'checked' : ''}> 
                                        <a href="/product/detail/${item.productId}" class="position-absolute">
                                            <span class="badge badge-danger m-3">${item.product.discount}%</span>
                                        </a>
                                        <div class="d-flex  align-items-center p-3">
                                            <a href="/product/detail/${item.productId}"><img src="/uploads/images/${item.product.image}"
                                                    class="img-fluid"></a>
                                            <a href="/product/detail/${item.productId}"
                                                ...
                                                <p class="text-muted mb-2">
                                                    <del
                                                    class="text-success mr-1">$<fmt:formatNumber type="number" maxFractionDigits="2" value="${item.product.unitPrice * (1 + item.product.discount/100)}"/></del> $<fmt:formatNumber type="number" maxFractionDigits="2" value="${item.product.unitPrice}"/>
                                                </p>
                                                <div class="d-flex align-items-center">
                                                    <p class="total_price font-weight-bold m-0">$<fmt:formatNumber type="number" maxFractionDigits="2" value="${item.product.unitPrice * item.quantity}"/></p>
                                                    <form id='myform' class="cart-items-number d-flex ml-auto" onsubmit="event.preventDefault()">
                                                        <input onclick="changeQuantity('${item.cartItemId}', 'quantity${item.productId}', -1)" type='button' value='-'
                                                            class='qtyminus btn btn-success btn-sm'
                                                            field='quantity${item.productId}' />
                                                        <input onclick="event.preventDefault()" onchange="changeQuantity('${item.cartItemId}', 'quantity${item.productId}')" type='text' name='quantity${item.productId}' value='${item.quantity}'
                                ...
                                <div>
                                    <a href="#" class="text-decoration-none btn btn-block p-3" type="button"
                                        ...
                                        <div
                                            ...
                                            <div class="more">
                                                <h6 class="m-0">Subtotal $${totalPrice}</h6>
                        ...
                        <h2 class="mb-0">
                            <button
                                ...
                                <span class="c-number">2</span> Order Address <a href="#" data-toggle="modal"
                                    ...
                                    class="text-decoration-none text-success ml-auto"> <i
                                        class="icofont-plus-circle mr-1"></i>Add New Delivery Address</a>
                            </button>
                        </h2>
                    </div>
                ...
                <div class="card border-0 osahan-accor rounded shadow-sm overflow-hidden mt-3">
                    ...
                    <div id="collapsefour" class="collapse" aria-labelledby="headingfour"
                        ...
                        <div class="card-body px-3 pb-3 pt-1 border-top">
                            ...
                            <a href="/checkout" class="btn btn-success btn-lg btn-block mt-3"
        ...
        <div class="sticky_sidebar">
            <div class="bg-white rounded overflow-hidden shadow-sm mb-3 checkout-sidebar">
                <div class="d-flex align-items-center osahan-cart-item-profile border-bottom bg-white p-3">
                    <img alt="osahan" src="/assets/client/img/starter1.jpg" class="mr-3 rounded-circle img-fluid">
                    <div class="d-flex flex-column">
                        <h6 class="mb-1 font-weight-bold">Tshop Online Store</h6>
                        <p class="mb-0 small text-muted"><i class="feather-map-pin"></i> Dong Thap . Viet Nam</p>
                    </div>
                </div>
                <div>
                    <div class="bg-white p-3 clearfix">
                        <p class="font-weight-bold small mb-2">Bill Details</p>
                        <p class="mb-1">Item Total <span class="small text-muted">(${cartItems.size()} item)</span> <span
                                class="float-right text-dark">$<fmt:formatNumber type="number" maxFractionDigits="2" value="${totalPrice}"/> </span></p>
                        <p class="mb-1">VAT (included in the product price)  <span class="float-right text-dark">$0</span></p>
                        <p class="mb-3">
                            Ship <span data-toggle="tooltip" data-placement="top"
                                title="Delivery partner fee - $1" class="text-info ml-1"><i class="icofont-info-circle"></i></span>
                                <span class="float-right text-dark">Free</span>
                        </p>
                        <h6 class="mb-0 text-success">
                            Total Discount<span
                                class="float-right text-success">$<fmt:formatNumber type="number" maxFractionDigits="2" value="${totalPrice}"/></span>
                        </h6>
                    </div>
                    <div class="p-3 border-top">
                        <h5 class="mb-0">TO PAY <span class="float-right text-danger">$<fmt:formatNumber type="number" maxFractionDigits="2" value="${totalPrice}"/></span></h5>
                    </div>
                </div>
            </div>
            <p class="text-success text-center">Your Total Savings on this order 
                $<fmt:formatNumber type="number" maxFractionDigits="2" value="${totalSaving}"/>
            </p>
        </div>
    </div>
</div>
